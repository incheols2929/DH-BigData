...
class PgUtils(object):
    ...
    def insertDB(self,schema,table,colum,data):
        ...
        try:
            ...
            write_log(" insert DB err ",e) 
    
    def readDB(self,schema,table,colum,where):
        sql = " SELECT {colum} from {schema}.{table} WHERE {where}".format(colum=colum,schema=schema,table=table,where=where)
        try:
            self.cursor.execute(sql)
    ...
    def updatInserteDB(self,schema,table,colum,data,conflict):
        ...
        try :
            ...
            write_log(" update DB err",e)
            self.db = psycopg2.connect(host=self.host, port=self.port, dbname=self.dbname, user=self.user, password=self.password)
            self.cursor = self.db.cursor()