from config import config_nlp as con
...
class Processing():
        ...
        for pattern in pattern_list:
            ...
            for iteration_term in iteration_term_list:
                sentence = sentence.replace(iteration_term,
                                            iteration_term[:pattern.count(".")] + replace_char * (
                                                        len(iteration_term) - pattern.count(".")))
        return sentence
    ...
    def replacer(self, sentence):
        patterns = [
            ...
            (r'i\'m', 'i am'),
            (r'ain\'t', 'is not'),
            (r'(\w+)\'ll', r'\1 will'),  # \g<1> 를 \1 로 변경
            (r'(\w+)n\'t', r'\1 not'),  # \g<1> 를 \1 로 변경
            (r'(\w+)\'ve', r'\1 have'),  # \g<1> 를 \1 로 변경
            (r'(\w+)\'s', r'\1 is'),  # \g<1> 를 \1 로 변경
            (r'(\w+)\'re', r'\1 are'),  # \g<1> 를 \1 로 변경
            (r'(\w+)\'d', r'\1 would'),  # \g<1> 를 \1 로 변경
        ]
